...
def generate_pdf_report(input_data, log_pred, log_prob, rf_pred, rf_prob, best_pred, best_model, best_conf):
    ...
    \textbf{Logistic Regression}: """ + ("Has Heart Disease" if log_pred == 0 else "No Heart Disease") + r""" (Confidence: """ + f"{log_prob if log_pred == 0 else 1 - log_prob:.2f}" + r""") \\
    \textbf{Random Forest}: """ + ("Has Heart Disease" if rf_pred == 0 else "No Heart Disease") + r""" (Confidence: """ + f"{rf_prob if rf_pred == 0 else 1 - rf_prob:.2f}" + r""") \\
...
if submit_button:
    ...
    if any(input_data.iloc[0].isna()) or age < 0 or trestbps < 50 or chol < 50 or thalach < 50 or oldpeak < 0:
        ...
        with st.spinner("Analyzing patient data..."):
            ...
            rf_prob = random_forest_model.predict_proba(input_data)[0][1]  # Probability of class 1
            rf_prob_no = 1 - rf_prob  # Probability of class 0
            ...
            st.write(f"**Logistic Regression**: {'Has Heart Disease' if log_pred == 0 else 'No Heart Disease'} (Confidence: {log_prob_no if log_pred == 0 else log_prob:.2f})")
            st.write(f"**Random Forest**: {'Has Heart Disease' if rf_pred == 0 else 'No Heart Disease'} (Confidence: {rf_prob_no if rf_pred == 0 else rf_prob:.2f})")
            
            # Decide best prediction
            if log_pred == rf_pred:
                best_pred = log_pred
                best_model = "Both Models"
                best_conf = max(log_prob_no if log_pred == 0 else log_prob, rf_prob_no if rf_pred == 0 else rf_prob)
            else:
                if log_prob_no if log_pred == 0 else log_prob > rf_prob_no if rf_pred == 0 else rf_prob:
                    best_pred = log_pred
                    best_model = "Logistic Regression"
                    best_conf = log_prob_no if log_pred == 0 else log_prob
                else:
                    best_pred = rf_pred
                    best_model = "Random Forest"
                    best_conf = rf_prob_no if rf_pred == 0 else rf_prob
            ...
            if MATPLOTLIB_AVAILABLE:
                ...
                confidences = [log_prob_no if log_pred == 0 else log_prob, rf_prob_no if rf_pred == 0 else rf_prob]
